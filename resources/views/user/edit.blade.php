        ...
        <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-4xl">
            ...
            <form action="{{ route('user.update') }}" method="POST" class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="first-name" name="firstname" value="{{ old('firstname', $user->firstname) }}"
                        required
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="last-name" name="lastname" value="{{ old('lastname', $user->lastname) }}"
                        required
                        class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="companyname" id="english" placeholder="Enter your company name"
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" id="nepaliInput" placeholder="Type in English..." name="companyname_np"
                        class="mt-2 w-full px-4 py-4 placeholder:text-sm border border-gray-300 rounded-md shadow-sm focus:ring-0 focus:border-orange-500 sm:text-base"
                        oninput="transliterateText()"
                        autocomplete="off"value="{{ old('companyname_np', $user->companyname_np) }}">
                </div>
                ...
                <div class="flex gap-6 sm:col-span-2">
                    ...
                    <div class="w-full sm:w-1/2 mb-4">
                        ...
                        <select id="type" name="category" required
                            class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400">
                            <option value="it" {{ old('category', $user->category) == 'it' ? 'selected' : '' }}>IT
                                Company</option>
                            <option value="sales" {{ old('category', $user->category) == 'sales' ? 'selected' : '' }}>
                                Sales Company</option>
                            <option value="bank" {{ old('category', $user->category) == 'bank' ? 'selected' : '' }}>Bank
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="w-full sm:w-1/2 mb-4">
                        ...
                        <select id="company_type" name="type" required
                            class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400">
                            <option value="single" {{ old('type', $user->type) == 'single' ? 'selected' : '' }}>Single
                                Shareholder Company</option>
                            <option value="multiple" {{ old('type', $user->type) == 'multiple' ? 'selected' : '' }}>Multi
                                Shareholder Company</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex justify-between sm:col-span-2">
                    ...
                    <div>
                        <a href="{{ route('user.index') }}"
                            class="w-full sm:w-auto bg-gray-500 text-white p-3 rounded-lg hover:bg-gray-700 transition duration-200">Cancel</a>
                    </div>

                    <!-- Submit Button -->
                    <div>
                        <button type="submit"
                            class="w-full sm:w-auto bg-orange-600 text-white p-3 rounded-lg hover:bg-orange-700 transition duration-200">Update
                            Profile</button>
                    </div>
                </div>
            ...
            timeout = setTimeout(async () => {
                ...
                try {
                    let response = await fetch(
                        `https://inputtools.google.com/request?text=${encodeURIComponent(text)}&itc=ne-t-i0-und&num=1`
                    );
